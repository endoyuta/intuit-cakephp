...
class AggcatAuthComponent extends Component {

	public static function GetOAuthTokens(&$oauth_token, &$oauth_token_secret) {
		AggcatAuthComponent::PrepSAMLAssertion($saml_xml_request);
		AggcatAuthComponent::PrepOAuthViaSAML($saml_xml_request, $oauth_token, $oauth_token_secret);
	}
	...
	public static function PrepSAMLAssertion(&$saml_xml_request) {
		...
		$saml_cert_path = Configure::read('Intuit.saml_x509_cert_path');
		$saml_prv_key_path = Configure::read('Intuit.saml_x509_private_key_path');

		$SAMLParams = array();
		$SAMLParams['IssueInstant'] 	= $DateTimeNowString;
		$SAMLParams['Issuer'] 			= Configure::read('Intuit.saml_identity_provider_id');
		$SAMLParams['ID']				= SimpleSAML_Utilities::generateID();
		$SAMLParams['NameID']			= Configure::read('Intuit.saml_name_id');
		...
		$SAMLParams['Audience']			= Configure::write('Intuit.saml_identity_provided_id');
		$SAMLParams['x509']				= file_get_contents($saml_cert_path);
		$SAMLParams['private_key']		= file_get_contents($saml_prv_key_path);
